...
void TestBitArrayValues(int bit_width, int num_vals) {
  ...
  BitWriter writer(buffer.get(), len);
  ...
  BitReader reader(buffer.get(), len);
  for (int i = 0; i < num_vals; ++i) {
    int64_t val = 0;
...
TEST(BitRle, Overflow) {
  for (int bit_width = 1; bit_width < 32; bit_width += 3) {
    ...
    RleEncoder encoder(buffer.get(), len, bit_width);
    ...
    RleDecoder decoder(buffer.get(), bytes_written, bit_width);
    parity = true;
    uint32_t v;
